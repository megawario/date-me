...
3. Customize `brand`, `headline`, `prompt`, `qrLabel`, `foregroundColor`, `backgroundColor`, and `accentColor`. Set `artwork` to a relative local image path, or to an empty string for no artwork, and describe it with `artworkAlt`.
