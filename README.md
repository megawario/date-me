...
Fun Mario uses `assets/images/fun-mario.gif` with `assets/images/fun-mario.jpg` as its reduced-motion fallback and includes a Spotify album card configured in `data/profiles.json`; the remaining photo slots use the local placeholder image so real photos can be added later by changing only the configuration.
